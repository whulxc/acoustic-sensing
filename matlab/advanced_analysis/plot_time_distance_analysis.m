% Optional downstream analysis for the time-distance CC map.
%
% This legacy helper is not called by the default public example. It keeps the
% earlier exploratory analysis stages:
%   1. large-scale distance-frequency power map;
%   2. local time-distance CC map;
%   3. ridge and local-maximum extraction;
%   4. local distance-frequency power map.
%
% Inputs:
%   Frame_signal_CC_hilbert    multi-frame CC-envelope sequence
%   transmit_delay             transmit-delay sample index for each frame
%   start_transmit_delay       reference delay sample index
%   select_sub                 true for one 2-by-2 figure, false for figures
%   startDist2DistStartIndex   local distance-window start, in meters
%   endDist2DistEndIndex       local distance-window end, in meters
%   start_frame/end_frame      frame range to analyze
%   caxis1_max/caxis2_max      optional color-axis limits
%
% Outputs:
%   near_ridge_result          local peaks nearest to the ridge
%   ridge_result               extracted ridge track
%   X_t/Y_dis/Z_CC             time axis, distance axis, and CC map
%   figure1..figure4           generated figures
...
ridge_result=[];
    ...
    switch nargin
        case 1:5
            error('Not enough input arguments for plot_time_distance_analysis.');
        case 6
            start_frame=1;
        ...
        case 10  
        otherwise
            error('Too many input arguments for plot_time_distance_analysis.');
    end
    %% 构建多帧CC序列 %%
    frame_ith=0;
